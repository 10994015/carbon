...
if (isset($_SESSION['logged_in'])) {
		$query = $pdo->prepare('SELECT * FROM other');
		$query->execute();
...
<body>
	<div class="container">
	<a href="index.php" title="" id="logo">其他碳材料專區</a>
		<br />
		<ol>
		...
		<?php if ($articles): ?>
			<?php foreach ($articles as $other):?>
				<?php echo '<li>' ?>
				<?php echo $other['article_title']; ?>
				<?php echo "<a href='edit.php?id=". $other['article_id'] ."'>編輯</a>"?>
				<?php echo "<button onclick='del_func(". $other['article_id'].")'>刪除</button>"?>
				<?php echo '<br>' ?>
				<?php echo '</li>' ?>
