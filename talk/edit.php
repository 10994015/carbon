...
if (isset($_SESSION['logged_in'])) {
		...
		$id = $_GET['id'];
		$query = $pdo->prepare('SELECT * FROM talk WHERE id = ?');
		$query->bindValue(1,$id);

		$query->execute();

		$talk = $query->fetch(PDO::FETCH_ASSOC);
...
<body>
	...
	<form method="POST" action="includes/edit.php?id=<?php echo $id; ?>" accept-charset="utf-8">
		文章標題：<textarea type="text" name="title"><?php echo $talk['text']; ?></textarea>
		<br>
		文章內容：<textarea name="content"><?php echo $talk['text2']; ?></textarea>
<br>
		<button type="submit" onclick=del_func(<?php echo $talk['id']?>)>確定</button>
	</form>
