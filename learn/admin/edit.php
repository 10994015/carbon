...
if (isset($_SESSION['logged_in'])) {
		...
		$id = $_GET['id'];
		$query = $pdo->prepare('SELECT * FROM learn WHERE article_id = ?');
		$query->bindValue(1,$id);

		$query->execute();

		$learn = $query->fetch(PDO::FETCH_ASSOC);
...
<body>
	...
	<form method="POST" action="../includes/edit.php?id=<?php echo $id; ?>" accept-charset="utf-8">
		文章標題：<input type="text" name="title"   value=<?php echo $learn['article_title']; ?>>
		<br>
		文章內容：<textarea name="content"><?php echo $learn['article_content']; ?></textarea>
<br>
